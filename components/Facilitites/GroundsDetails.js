...
import { getCollege, getGalleryCategory, getGrounds } from "../../Util/auth";
      ...
      <Pressable style={styles.itemContainer2}>
        ...
        >
          <Image
            ...
            source={{
              uri: "http://mobile.khelsathi.in/files/grounds/" + item.image,
            }}
          />
          <View
            style={{
...
const GroundsDetials = ({ navigation, state, data, image }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await getGrounds();
